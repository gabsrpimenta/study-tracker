// Página de Login: formulário para o utilizador autenticar-se.
...
export default function Login() {
  ...
  // ==========================================
  // 2. FUNÇÃO DE SUBMISSÃO DO LOGIN
  // ==========================================
  async function handleSubmit(e) {
    e.preventDefault(); // Evita que a página recarregue ao dar Enter/Submit
    setLoading(true);
    
    try {
      await login(email, senha); // Tenta fazer a autenticação
      toast.success("Login efetuado com sucesso!");
      navigate("/"); // Se a senha estiver correta, redireciona para o Dashboard ("/")
    } catch (err) {
      toast.error(err.message); // Exibe o erro devolvido pela API (ex: "Senha incorreta")
    } finally {
      setLoading(false); // Retira o "loading" no final
    }
  }
  ...
  // ==========================================
  return (
    // O fundo mantém-se estaticamente branco usando 'bg-white'
      ...
      <div className="relative z-10 w-full max-w-md space-y-8 p-10 bg-white rounded-3xl border border-slate-200 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] transition-all duration-300 group opacity-0 st-animate-card">
        ...
        <div className="flex flex-col items-center space-y-2 text-center">
          ...
          <p className="text-sm text-slate-500 opacity-0 st-animate-title">
            Bem-vindo de volta ao teu centro académico.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 opacity-0 st-animate-form">
          ...
          <div className="text-center pt-3 border-t border-slate-100 mt-5">
            <p className="text-sm text-slate-500">
              Ainda não tens conta? <Link to="/cadastro" className="font-semibold text-primary underline-offset-4 hover:underline transition-all">Cadastra-te grátis</Link>
            </p>
          </div>
        </form>
      </div>
