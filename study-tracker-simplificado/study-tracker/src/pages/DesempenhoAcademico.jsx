// Das bibliotecas puxamos a magia e o talento,
// Para dar ao ecrã muito mais movimento.
...
// No palco do Desempenho Académico,
// Cada nota e cadeira tem um valor sistémico.
export default function DesempenhoAcademico() {
  // A memória guarda o que a página sente,
  // O que carrega, o que edita e o que está presente.
  const [isLoading, setIsLoading] = useState(true);
  const [activeTab, setActiveTab] = useState("disciplinas"); 
  const [subjects, setSubjects] = useState([]); // As nossas Cadeiras
  const [grades, setGrades] = useState([]);     // As nossas Avaliações/Notas

  // Estados que controlam a nossa janela flutuante (Modal)
  const [open, setOpen] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [form, setForm] = useState({});

  // Ao abrir a cortina, sem hesitar,
  // As cadeiras e notas vamos logo buscar.
  useEffect(() => {
    setIsLoading(true);
    Promise.all([
  ...
  // Para criar o novo ou o antigo mudar,
  // Limpamos o formulário antes de gravar.
  ...
  // Se a ideia é editar, a caneta entra em ação,
  // Puxando os dados da cadeira ou avaliação.
  ...
  // Guardamos o esforço com muita atenção,
  // Ligando a interface ao nosso coração (a Base de Dados).
  async function handleSave() {
    try {
      if (activeTab === "disciplinas") {
        if (editingId) await API.updateSubject(editingId, form);
        else await API.createSubject(form);
        setSubjects(await API.listSubjects()); // Atualiza a lista
      } else {
        // Garantimos que os valores numéricos são guardados como números
        ...
        setGrades(await API.listGrades()); // Atualiza a lista
      }
  ...
  // Se algo está a mais e queremos limpar,
  // Com um simples clique vamos apagar.
  ...
  // Enquanto a magia acontece no fundo,
  // Um ícone gira e encanta o mundo.
  if (isLoading) {
    return (
  ...
  // Desenhamos com luz o nosso painel,
  // Cada nota guardada, doce como mel.
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700 pb-10 px-2 md:px-6">
              ...
              subjects.map(s => (
                <Card key={s.id || s.Id} className="rounded-[1.5rem] p-6 border shadow-sm transition-all hover:border-primary/20">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {/* Botões de Ação (escondidos até passares o rato) */}
                ...
                grades.map(g => (
                  <div key={g.id || g.Id} className="flex items-center justify-between p-4 rounded-2xl border bg-card hover:bg-muted/30 hover:border-primary/30 transition-all group">
                    ...
                    <div className="flex items-center gap-4">
                      {/* O valor da nota ganha destaque com a Badge */}
      ...
      {/* MODAL MÁGICO PARA CRIAR/EDITAR */}
      <Dialog 
        open={open} 
